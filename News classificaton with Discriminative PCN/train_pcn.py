...
from pcn_model_original import PCN ## bring in model from museum      _original
...
dataX = "../../data/ag-news/teain_x.npy"
dataY = "../../data/ag-news/train_y.npy"
devX = "../../data/ag-news/test_x.npy"
devY = "../../data/ag-news/test_y.npy"
...
model = PCN(subkeys[1], x_dim, y_dim, hid1_dim=256, hid2_dim=16, T=20,
            dt=1., tau_m=20., act_fx="relu", eta=0.001, exp_dir="exp",
            model_name="pcn")
...
for i in range(n_iter):
    ...
    for j in range(n_batches):
        ...
        # print("min: ", jnp.min(_X), "max: ",jnp.max(_X),"st deviation", jnp.std(_X), "shape: ", _X.shape)  # Check min, max, and standard deviation
        # for j in range(n_batches):
        #     idx = j * mb_size
        #     Xb = X[idx: idx + mb_size, :]
        #     print(f"Batch {j} mean: {jnp.mean(Xb)}")
