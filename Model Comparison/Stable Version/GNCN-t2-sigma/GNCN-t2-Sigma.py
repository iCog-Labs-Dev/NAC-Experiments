...
class GNCN_Sigma:
    ...
    the GNCN-T2-Lsigma, according to the naming convention in
    (Ororbia & Kifer 2022).
    ...
    Args:
        args: a Config dictionary containing necessary meta-parameters for the GNCN-T2-Lsigma
    ...
    def __init__(self, args):
        ...
        ngc_model = NGCGraph(K=K, name="gncn_sigma")
    ...
    def get_top_latents(self, x):
        """
        Get the topmost latent representations for input x.
        
        Args:
            x: input data
            
        Returns:
            z_top: topmost latent representations
        """
        readouts, _ = self.ngc_model.settle(
            clamped_vars=[("z0","z", x)],
            readout_vars=[("mu2","phi(z)")],  # Get topmost latent activations
            calc_delta=False
        )
        z_top = readouts[0][2]
        return z_top
    ...
    def update(self, x):
        """
        Updates synaptic parameters/connections given inputs x
        ...
        """
        x_hat = self.settle(x, calc_update=True)
        if self.delta is not None:
            delta = self.calc_updates(avg_update=True)
            # Apply gradients and constraints
    ...
    def predict(self, x):
        ...
        """
        # Assuming 'settle' method returns the predicted output
        predictions = self.settle(x, calc_update=False)
        return predictions
